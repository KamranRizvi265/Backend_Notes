...
def api_request(method, path, **kwargs):
    """Send a request to the backend and surface connection errors in the UI."""
    try:
        return requests.request(method, f"{API_BASE_URL}{path}", timeout=10, **kwargs)
    except requests.exceptions.ConnectionError:
        st.error(f"Backend is not running at {API_BASE_URL}. Start the FastAPI app first.")
        return None
...
def feed_page():
    ...
    if response.status_code == 200:
        ...
        posts = payload.get("posts") or payload.get("post") or []
